...
if os.path.exists(ZIP_PATH) and _zip_on_uudempi():
    ...
    try:
        ...
        print("⚠️  ethaku.csv on käytössä toisessa ohjelmassa – käytetään olemassa olevaa versiota.\n")
      ...
      <div class="border border-violet-100 rounded-xl p-4 bg-violet-50/40">
        ...
        <div class="flex items-center justify-between gap-2 mb-3">
          ...
          <div class="flex gap-1.5">
            <button onclick="avaaGoogleHaku()"
              class="flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all whitespace-nowrap">
...
function avaaGoogleHaku() {
  if (!currentRow) return;
  const osat = [
    currentRow.nimi,
    currentRow.osoite,
    currentRow.alakaytto || currentRow.kayttotarkoitus,
    'omistaja'
  ].filter(Boolean);
  const q = encodeURIComponent(osat.join(' '));
